...
impl Png {
    ...
    pub fn remove_first_chunk(&mut self, chunk_type: &str) -> Result<Chunk> {
        ...
        {
            return Ok(self.0.remove(i));
        }
        Err(Error::from("couldn't find specified chunk_type"))
    }
    ...
    pub fn as_bytes(&self) -> Vec<u8> {
        ...
        self.chunks()
            .iter()
            .for_each(|c| bytes.extend(c.as_bytes()));
        bytes
    }
}
...
impl TryFrom<&[u8]> for Png {
    ...
    fn try_from(bytes: &[u8]) -> Result<Png> {
        ...
        while let Ok(()) = reader.read_exact(&mut buf) {
            let end_pos = 4 + u32::from_be_bytes(buf) + 4;
            let mut chunk_buf = vec![0; usize::try_from(end_pos)?];
            reader.read_exact(&mut chunk_buf)?;
            let chunk_bytes: Vec<u8> = buf
                .iter()
                .cloned()
                .chain(chunk_buf.iter().cloned())
                .collect();
